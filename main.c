...
void main_atualiza_reserva_txt() {
    //variaveis para atualizar reserva
    int pos_antiga_aco = 0, cod_hos = 0, pos_hos = 0, disponivel = 0, cod_aco = 0, ret_data = 0, ret_reserva = 0, retorno = 0, contador = 0, pos_cat = 0;
    int sair = 0, qtd_aco = 0, qtd_res = 0, pos_res = 0, contador_erro = 0, pos_aco = 0, qtd_hos = 0, cod_res = 0, opcao = 0, qtd_cat = 0;
    ...
    printf("\n--------Atualizar reservas----------\n");
    printf("Digite o codigo da reserva que deseja atualizar:");
    scanf("%d", &cod_res);
    pos_res = procura_reserva_txt(cod_res, qtd_res, reservasRAM);
    while (pos_res == -1) {
        //pedindo ao usuario o codigo da reserva que deseja alterar
    ...
    printf("Data do check-out:%d/%d/%d\n", reservasRAM[pos_res].datas.dia_out, reservasRAM[pos_res].datas.mes_out, reservasRAM[pos_res].datas.ano_out);
    printf("Código do hóspede:%d\n\n", reservasRAM[pos_res].cod_hospede);
    printf("Deseja trocar a data de check-in e check-out?(1-Sim/0-Não):");
    scanf("%d", &opcao);
    if (opcao == 1) {
        printf("Essa acomodação não esta disponivel nessas datas,lembrando que uma dessas datas é a da própria reserva que esta sendo atualizada.\n");
        procura_data_reserva_txt(qtd_res, reservasRAM[pos_res].codigo_aco, reservasRAM);
        printf("Digite uma data para fazer a reserva que não seja esssas datas mostradas.\n");
        printf("Dia do Chekin:");
        scanf("%d", &datas.dia_in);
        printf("Mês do Chekin:");
        ...
        //verifico se a data que o usuario digitou nao bate com nenhuma outra
        ret_reserva = verifica_data_txt(qtd_res, reservasRAM, datas, reservasRAM[pos_res].codigo_aco);
        while (ret_reserva == -1) {
            printf("Voce digitou uma data que ja esta reservada.\nDigite outra data\n");
            printf("Dia do Chekin:");
            scanf("%d", &datas.dia_in);
        ...
        reservasRAM[pos_res].datas.dia_in = datas.dia_in;
        reservasRAM[pos_res].datas.mes_in = datas.mes_in;
        reservasRAM[pos_res].datas.ano_in = datas.ano_in;
        reservasRAM[pos_res].datas.dia_out = datas.dia_out;
        reservasRAM[pos_res].datas.mes_out = datas.mes_out;
        reservasRAM[pos_res].datas.ano_out = datas.ano_out;
        contador++;
    }


    printf("Deseja trocar a acomodação?(1-Sim/0-Não):");
    scanf("%d", &opcao);
    if (opcao == 1) {
        opcao = 0;
        printf("Digite o codigo da acomodação que deseja:");
        scanf("%d", &cod_aco);
        if (cod_aco == reservasRAM[pos_res].codigo_aco) {
            disponivel = -1;
            printf("Essa é a acomodação atual,");
        } else {
            disponivel = verifica_ocupacao_txt(qtd_aco, cod_aco, acomodacaoRAM);
        }
        //se retornar -1 é pq a acomodacao nao existe
        while (disponivel == -1) {
            contador_erro++;
            printf("Digite uma acomodacao válida:");
            scanf("%d", &cod_aco);
            if (cod_aco == reservasRAM[pos_res].codigo_aco) {
                disponivel = -1;
                printf("Essa é a acomodação atual,");
        ...
        //traz a posicao da acomodacao que o usuario escolheu
        pos_aco = procura_acomodacao_txt(cod_aco, qtd_aco, acomodacaoRAM);
        //se disponivel igual a 0 quer dizer que a acomodacao esta disponivel para qualquer momento
        if (disponivel == 0) {
            pos_antiga_aco = procura_acomodacao_txt(reservasRAM[pos_res].codigo_aco, qtd_aco, acomodacaoRAM);
            acomodacaoRAM[pos_antiga_aco].ocupacao--;
            acomodacaoRAM[pos_aco].ocupacao++;
            atualiza_acomodacoes_txt(qtd_aco, acomodacaoRAM);

            reservasRAM[pos_res].codigo_aco = acomodacaoRAM[pos_aco].codigo;
            strcpy(reservasRAM[pos_res].descricao, acomodacaoRAM[pos_aco].descricao);
            strcpy(reservasRAM[pos_res].facilidades, acomodacaoRAM[pos_aco].facilidades);
            reservasRAM[pos_res].categoria.codigo = acomodacaoRAM[pos_aco].categoria;

            pos_cat = procura_categoria_txt(reservasRAM[pos_res].categoria.codigo, qtd_cat, categoriaRAM);

            strcpy(reservasRAM[pos_res].categoria.descricao, categoriaRAM[pos_cat].descricao);
            reservasRAM[pos_res].categoria.valor_diaria = categoriaRAM[pos_cat].valor_diaria;
            reservasRAM[pos_res].categoria.adultos = categoriaRAM[pos_cat].adultos;
            reservasRAM[pos_res].categoria.criancas = categoriaRAM[pos_cat].criancas;
            ...
            retorno = verifica_data_txt(qtd_res, reservasRAM, datas, cod_aco);
            if (retorno == -1) {
                printf("\nEssa acomodação não esta disponível para a data atual da reserva.\n");
                printf("Digite a opção desejada.\n");
                printf("1-Deseja escolher outra acomodação para atualizar a reserva.\n");
                printf("2-Deseja continuar a atualização da reserva mas manter a acomodação.\n");
                printf("3-Deseja alterar a data para se encaixar nessa acomodação\n");
                printf("4-Deseja cancelar a atualização da reserva:");
                scanf("%d", &opcao);
                if (opcao == 2) {
                    contador--;
                }
                if (opcao == 3) {
                    printf("Essa acomodação não esta disponivel nessas datas,lembrando que uma dessas datas é a da própria reserva que esta sendo atualizada.\n");
                    procura_data_reserva_txt(qtd_aco, cod_aco, reservasRAM);
                    printf("Digite uma data para fazer a reserva que não seja esssas datas mostradas.\n");
                    printf("Dia do Chekin:");
                    scanf("%d", &datas.dia_in);
                    printf("Mês do Chekin:");
                ...
                int pos_antiga_cat,dias_antigo,dias_novo;
                
                //mudando os valores da acomodacao antiga
                dias_antigo=contarDias(reservasRAM[pos_res].datas);
                pos_antiga_aco = procura_acomodacao_txt(reservasRAM[pos_res].codigo_aco, qtd_aco, acomodacaoRAM);
                pos_antiga_cat = procura_categoria_txt(reservasRAM[pos_res].categoria.codigo,qtd_cat,categoriaRAM);
                
                valor_antigo=dias*categoriaRAM[pos_antiga_cat].valor_diaria;
                
                acomodacaoRAM[pos_antiga_aco].ocupacao--;
                acomodacaoRAM[pos_antiga_aco].diarias-=dias;
                acomodacaoRAM[pos_antiga_aco].lucro-=valor;
    ...
    printf("Deseja trocar o hóspede?(1-Sim/0-Não):");
    scanf("%d", &opcao);
    if (opcao == 1) {
        printf("Digite o codigo do hospede que deseja fazer a reserva?");
        scanf("%d", &cod_hos);
        pos_hos = procura_hospede_txt(cod_hos, qtd_hos, hospedesRAM);
        //loop para o usuario digitar um codigo de hospede valido
        while (pos_hos == -1) {
            contador_erro++;
            printf("Digite um codigo de hospede valido:");
            scanf("%d", &cod_hos);
            pos_hos = procura_hospede_txt(cod_hos, qtd_hos, hospedesRAM);
            if (contador_erro > 2) {
                printf("Ocorreu muitos erros deseja sair?(1-Sim/0-Não)");
                scanf("%d", &sair);
                contador_erro = 0;
            }
            if (sair != 0) {
        ...
        reservasRAM[pos_res].cod_hospede = cod_hos;
        contador++;
    }
    if (contador > 0) {
        atualiza_reserva_txt(qtd_res, reservasRAM);
    }
    menu_modulo_reservas();
}
 */
